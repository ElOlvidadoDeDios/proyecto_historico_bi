...
CTE_REPAGO AS (
    ...
    INNER JOIN SEGURIDAD.dbo.ANAREC T_ANA ON T_ANA.ID_ANAREC = T_PRE.ID_ANA AND T_ANA.FLAG_ANAREC = 'A'
    INNER JOIN SEGURIDAD.dbo.USUARIOS T_USU ON T_USU.ID_USER = T_ANA.ID_USER
    WHERE FORMAT(T_MOV.FECHA_MOV, 'yyyyMM') = :periodo 
      AND T_MOV.TIPO_MOV != '0001' 
      AND T_MOV.TIPO_DOC IN ('01','03')
      -- 🌟 CANDADO DE SEGURIDAD EXCLUSIÓN DE EXTORNOS NATIVO:
      AND NOT EXISTS (
          SELECT 1
          FROM PREMOV A
          WHERE A.FECHA_MOV = T_MOV.FECHA_MOV 
            AND A.COD_AGE   = T_MOV.COD_AGE   
            AND A.COD_CAJA  = T_MOV.COD_CAJA  
            AND A.TIPO_DOC  = T_MOV.TIPO_DOC  
            AND A.NRO_DOC   = T_MOV.NRO_DOC   
            AND LEFT(A.TIPO_MOV, 2) = '01'
      )
