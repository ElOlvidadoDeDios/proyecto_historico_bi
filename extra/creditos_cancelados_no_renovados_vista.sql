CREATE VIEW dbo.VW_CREDITOS_CANCELADOS_NO_RENOVADOS
AS
WITH
CTE_AUX AS (
    SELECT
    ...
    FROM PREMOV
    WHERE FECHA_MOV >= DATEFROMPARTS(YEAR(DATEADD(MONTH, -2, GETDATE())), MONTH(DATEADD(MONTH, -2, GETDATE())), 1)
      AND FECHA_MOV <  DATEADD(MONTH, 1, DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1))
...
CTE_MAIN AS (
    ...
    INNER JOIN PRESTAMO                 NEXO_PRE ON NEXO_PRE.CUENTA    = T1.CUENTA
    ...
    WHERE
        T1.SALDO_PRES = 0
        AND CAST(T1.FEC_ULT_PAGO AS DATE) >= DATEFROMPARTS(YEAR(DATEADD(MONTH, -2, GETDATE())), MONTH(DATEADD(MONTH, -2, GETDATE())), 1)
        AND CAST(T1.FEC_ULT_PAGO AS DATE) <  DATEADD(MONTH, 1, DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1))
)
...
WHERE
    ...
    OR T.Next_Otorga > DATEADD(DAY, 7, T.Fecha_Ultimo_Pago);  -- 7 días = ventana de renovación
GO
